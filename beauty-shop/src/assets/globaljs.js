...
globaljs.install = function (Vue, options) {
  Vue.prototype.$xljs = {
    ...
    messageG () { // 全局消息通知数据，配合用户名，区分用户
      let that = this;
      return 'ls_global_message_g_data' + that.storageL(that.userId);
    },
    ...
    localProL (obj) {
      let that = this;
      let o = that.storageL(that.localPro) || {};
      if(obj instanceof Object){
        that.extend(o, obj);
        that.storageL(that.localPro, o);
      }
      return o;
    },
    // 将彩票数据储存在本地
    localLotL (obj) {
      let that = this;
      let o = that.storageL(that.localLot) || {};
      if(obj instanceof Object){
        that.extend(o, obj);
        that.storageL(that.localLot, o);
      }
      return o;
    },
    storageL (key, val) {
    ...
    formatUrl (url, tt, domain) {
        ...
        let userid = that.storageL(that.userId),
