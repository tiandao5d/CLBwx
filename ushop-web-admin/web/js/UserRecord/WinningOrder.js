...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('用户记录', '中奖记录', '用户管理后台');
...
function globalDataFn(callback) {
	...
	Common.ajaxAll([
		{url: '/ushop-web-admin/user/merchant/list'},
		{url: '/ushop-web-admin/spellBuy/winner/getConstants'}
	], function ( d1, d2 ) {
		globalData.listMerchantInfo = d1.listMerchantInfo || [];
		globalData.latestLotterysStatusList = d2.latestLotterysStatusList || [];
		callback();
	});
}
...
function detailsFn(orderNo) {
	...
	Common.dataAjax(url, function(data) {
		...
		randomNumber = data.latestLottery.randomNumber || ''; //期号
