...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('订单管理', '弃奖管理','夺宝管理后台');
...
function globalDataFn(callback){
	...
	Common.dataAjax(_url, function(data, status){
		if(status == 'success'){
//			globaldata.unclaimedLotterysStatusList = data.unclaimedLotterysStatusList;
...
//http://10.13.0.170:30005/ushop-web-admin/order/unclaimedLottery
...
//		str = '';
//	//订单列表详情
//	var url = Common.DOMAIN_NAME + '/ushop-web-admin/order/unclaimedLottery?orderNo=' + orderNo,
//		colModel = [
//			{label: '本期商品ID',name:'spellbuyProductId',index:'spellbuyProductId',width:120, align: 'center'},
//			{label: '商品ID',name:'productId',index:'productId',width:200, align: 'center'},
//			{label: '购买数量',name:'buyCount',index:'buyCount',width:80, align: 'center'},
//			{label: '商品期数',name:'productPeriod',index:'productPeriod', width:80, align: 'center'},
//			{label: '商品价格',name:'productPrice',index:'productPrice', width:80, align: 'center',
//				formatter: 'currency',
//				formatoptions: {
//					decimalPlaces: 2,
//					prefix : '￥'
//				}
//			},
//			{label: '商品名称',name:'productName',index:'productName',width:200, align: 'center'}
//		];
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'弃奖管理'});
}