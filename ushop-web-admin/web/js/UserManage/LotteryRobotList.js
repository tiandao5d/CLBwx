...
jQuery(function($) {
	...
	//菜单面包屑导航等配置显示
	Common.menuNavContent('用户管理', '拼购机器人列表' , '用户管理后台');
...
function determineClick(){
	...
	if(typeName == 'addAccount'){		
		...
		var _url = Common.DOMAIN_NAME + '/ushop-web-admin/robot/importGenerate';
...
//	var delImgUrl = Common.DOMAIN_NAME + '/ushop-web-admin/new/advertisement/deletePicture';
	var url1 = '/ushop-web-admin/user/robot/import';
...
//	    	}
		...
		var me = this,
			...
			if(/xls/.test(system)){
				...
				Common.jBoxNotice('只接受excel文件', 'red');
//				me.removeFile(_file);
			}
		});
...
function toExcel(n){
	if(n==1){	
		$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',aId:'exportExcel1',fileName:'机器人列表'});
	}else if(n==2){
		$('#robotModel').tableExport({type:'excel',escape:'false',aId:'exportExcel2',fileName:'机器人模板'});
	}
}