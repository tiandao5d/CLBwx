...
jQuery(function($) {
	...
	//菜单面包屑导航等配置显示
	Common.menuNavContent('商品管理', '商品审核','夺宝管理后台');
       	    ...
       	    $('#submit').css('display','block').unbind('click').click(function(){
       	    	...
       	    	Common.jBoxConfirm('商品审核','确定要审核通过吗？',function(){       	    		
	    			if(arguments[0] == 1){
		       	    	...
		       	    	Common.dataAjaxPost(_url, {id : id,productPrice:productPrice,singlePrice:singlePrice,spellbuyCount:spellbuyCount}, function(data){
							if(data.data == 'SUCCESS'){
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'商品审核'});
}