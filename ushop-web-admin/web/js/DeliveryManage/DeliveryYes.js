...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('发货管理', '已发货管理', '夺宝管理后台');
	...
	globalDataFn(function(data) {
		...
		$('.itemSearch').on('click', function() {
			...
			DeliveryYesList('#itemsTable1', '#itemsPager1', postData);
			//		}else{
			//			alert('结束时间要大于起始时间！');
			//		}
		});
		...
		DeliveryYesList('#itemsTable1', '#itemsPager1');
...
function detailsFn(id, type) {
	var url = Common.DOMAIN_NAME + '/ushop-web-admin/delivery/getById?id=' + id,
		str = '',
		imgStr = '';
