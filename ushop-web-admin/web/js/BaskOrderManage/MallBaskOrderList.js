...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('晒单管理', '商城晒单','夺宝管理后台');
	//日期选择器绑定
...
function globalDataFn(callback){
	...
	var _url = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/getConstants';
...
//http://10.35.0.66:8080/ushop-web-admin/new/share/sharelist
function baskOrderList(tableId, pagerId, postData){
	var gridUrl = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/list'
		colModel = [
			//{label: '标题',name:'title',index:'title',width:180, align: 'center'},
			{label: '商品名称',name:'object.productName',index:'object.productName',width:100, align: 'center'},
			...
			{label: '晒单人',name:'userName',index:'userName',width:120, align: 'center'},
			...
			{label: '操作', name:'',index:'', width:240,fixed:true, align: 'center',
				formatter: function(val, cellval , colpos, rwdat){
					var str = '';
					str += '<button class="btn btn-xs btn-primary" onclick="detailsFn(' + colpos.id + ')">晒单详情</button> ';
					if(colpos.status == 1){
						...
						str += '<button class="btn btn-xs btn-primary" onclick="detailsOrderFn(\'' + colpos.orderNo + '\')">订单详情</button> ';
					}
					return str;
				}
			}
...
//显示详情
//参数为数据的订单号
//http://10.35.0.66:8080/ushop-web-admin/new/share/shareInfo?id=2
function detailsFn(id, type){
	var url = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/getById?id=' + id,
		str = '', imgStr = '';
	var modal = $('#detailsModal'),
		shareDetail = modal.find('#shareDetail'),
		detailItem = modal.find('#detailItem'),
		tbody = shareDetail.find('tbody');
		shareDetail.removeClass('hide');
		detailItem.addClass('hide');
	modal.modal('show');
...
function detailsOrderFn(orderNo, type){
	var url = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/getByOrderNo?orderNo=' + orderNo,
		str = '', imgStr = '';
	...
	var modal = $('#detailsModal'),
		...
		shareDetail = modal.find('#shareDetail'),
		str = '';
		//modal.find('.expressInfo').addClass('hide');
		detailItem.removeClass('hide');
		shareDetail.addClass('hide');
	//订单列表详情
//	var url = Common.DOMAIN_NAME + '/ushop-web-portal/portal/mall/order/getOrderByOrderNo/' + orderNo;
	modal.modal('show');
	Common.dataAjax(url, function(data){
...
//		})
		$.each(withold,function(i,o){
			...
			if(i>0){
				trStr += '<tr><td>'+(o.productName||'')+'</td><td>'+o.number+'</td><td>￥'+str+'</td></tr>';
			}else{				
				trStr += '<tr><td>'+(o.productName||'')+'</td><td>'+o.number+'</td><td>￥'+str+'</td><td class="witholdStatus"></td><td class="sumTotal"></td></tr>';
			}
		...
		if(data.productOrder&&data.productOrder.expressCompany){
			...
			$.each(globalData.expressCompanyMap,function(i,o){
				if(o.code == data.productOrder.expressCompany){
...
function statusFn(id,status){
	...
	if(status == 2){
		content = '确定要审核通过吗？';
		_url = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/auditPassShare?id=' + id;
	}else if(status == 3){
		content = '确定要审核不通过吗？';
		_url = Common.DOMAIN_NAME + '/ushop-web-admin/mall/share/auditNoPassShare?id=' + id;
	}
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'商城晒单'});
}