...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('晒单管理', '积分抽奖晒单','夺宝管理后台');
	//日期选择器绑定
			...
			{label: '操作', name:'',index:'', width:240,fixed:true, align: 'center',
				formatter: function(val, cellval , colpos, rwdat){
					...
					str +=	'<button class="btn btn-xs btn-primary" onclick="statusFn(' + colpos.id +',3' +')">审核不通过</button> ';
						...
						str+='<button class="btn btn-xs btn-primary" onclick="detailsOrderFn(' + colpos.spellbuyProductId + ')">订单详情</button> ';
					}
					return str;
				}
			}
...
function detailsOrderFn(id, type){
	var url = Common.DOMAIN_NAME + '/ushop-web-admin/spellBuy/winner/getById?spellbuyProductId=' + id,
		str = '', imgStr = '';
	var modal = $('#detailsModal'),
		detailsItemSd = modal.find('.detailsItemSd'),
		tbody = detailsItemSd.find('tbody');
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'积分抽奖晒单'});
}