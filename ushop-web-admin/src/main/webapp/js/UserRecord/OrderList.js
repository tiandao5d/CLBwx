...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('用户记录', '夺宝记录','用户管理后台');
...
function globalDataFn(callback){
	...
	Common.dataAjax(_url, function(data, status){
		if(status == 'success'){
			globalData.PaymentStatusList = data.payStatusList;
			globalData.PaymentTypeList = data.payTypeList;
...
//http://10.13.0.170:30005/ushop-web-admin/order/orderList
function orderList(tableId, pagerId, postData, reload){
	var gridUrl = Common.DOMAIN_NAME + '/ushop-web-admin/order/list'
		colModel = [
//			{label: '交易时间',name:'buyTime',index:'buyTime',width:180, align: 'center'},
...
//			{label: '幸运号码',name:'randomNumber',index:'randomNumber', align: 'center'},
			{label: '用户ID',name:'userId',index:'userId', align: 'center',width:150,fixed:true},
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'夺宝记录'});
}