...
jQuery(function($) {
	//菜单面包屑导航等配置显示
	Common.menuNavContent('用户记录', '晒单记录','用户管理后台');
	...
	globalDataFn(function(data){	
		...
		$('.itemSearch').on('click', function(){
			var me = $(this),
				...
				productPeriod = box.find('[name=productPeriod]').val(),
				...
				productName = box.find('[name=productName]').val(),
				startDate = box.find('.start').val(),
				endDate = box.find('.end').val(),
...
function baskOrderList(tableId, pagerId, postData, reload){
	var gridUrl = Common.DOMAIN_NAME + '/ushop-web-admin/spellBuy/share/listByLatest?shareStatus=1'
		colModel = [
			...
			{label: '用户ID',name:'userId',index:'userId',width:100 , align: 'center'},
...
//					if(colpos.shareStatus == 2){
						str += '<button class="btn btn-xs btn-primary" onclick="detailsShareFn(' + colpos.shareId + ')">晒单详情</button> ';
//					}
					return str
				}
			}
	...
	Common.gridBackPaging(tableId, pagerId, gridUrl, colModel, postData, '已晒单列表');
}
...
function detailsShareFn(id, type){
	...
	Common.dataAjax(url, function(data){
		...
		for(var i=0;i<imgArr.length;i++){
			if(Common.oldImage.test(imgArr[i])){
				imgArr[i] = Common.IMAGE_URL + imgArr[i];
			}
			if(imgArr[i]){
...
//导出excel表格
function toExcel(){
	$('#gview_itemsTable1').tableExport({type:'excel',escape:'false',fileName:'晒单记录'});
}