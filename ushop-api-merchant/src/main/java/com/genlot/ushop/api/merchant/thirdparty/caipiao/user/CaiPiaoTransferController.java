...
@Controller
@RequestMapping(value = "/api/caipiao/user/transfer")
public class CaiPiaoTransferController{
	
	private static final Logger log = LoggerFactory.getLogger(CaiPiaoTransferController.class);
	...
	/**
	* 兑换
	* @example http://xxx.xxx.xxx.xxx:port/ushop-api-merchant/api/caipiao/user/transfer/submit/来源/目标/金额
	...
	*/
	...
	public Object submit(
		...
		try
		{
			...
			Integer province = 0;
			Integer platform = PlatformEnum.CAIPIAO.getValue();
			Integer type     = 0;
//			if (sourceFundType.getPlatformId().equals(PlatformEnum.CAIPIAO.getValue()) &&
//				targetFundType.getPlatformId().equals(PlatformEnum.CAIPIAO.getValue())) {
...
//			}
			...
			UserBindRelation relation = userBindRelationFacade.getByUserNo(
					...
					province, 
					platform,
					Integer.parseInt(sourceFundType.getGameId()));
			if (relation == null) {
				throw UserBizException.USER_BIND_RELATION_IS_NOT_EXIST;
			}
			
			// 获得本地账号
			Account source = accountQueryFacade.getAccountByUserNo_fundType(userNo, fromFundType);
			Account target = accountQueryFacade.getAccountByUserNo_fundType(userNo, toFundType);
			...
			order.setSourceFund(fromFundType);
			order.setTargetFund(toFundType);
...
//				order.setUrl(jiKaiPiaoParam.getWithdrawReturnUrl());
...
//				order.setUrl(jiKaiPiaoParam.getExchangeReturnUrl());
...
//			}
			...
			if (order.getTrxType().equals(AccountTradeTypeEnum.ACCOUNT_DEPOSIT.getValue())) {
				accountTransferFacade.transferIn(order.getTrxNo());
			}
	...
	*/
			...
			HttpServletResponse response) 
	{
	    ...
	    {
	    	log.info("CaiPiao deposit illegal call back: POST " + request.getRequestURL() + " " + queryString);  
	    }
			...
			HttpServletResponse response) 
	{
	    ...
	    if (queryString.isEmpty())
	    {
	    	log.info("CaiPiao withdraw illegal call back: POST " + request.getRequestURL() + " " + queryString);  
	    }
	    
	    queryString = queryString.substring(0, queryString.length() - 1);  
	    log.info("CaiPiao withdraw call back: POST " + request.getRequestURL() + " " + queryString);  
